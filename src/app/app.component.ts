...
export class AppComponent {
 ...
 private makeKnockout(){const size=this.nextPow2(this.players.length);const seeded:[string|null,...(string|null)[]]=[...this.players] as any;while(seeded.length<size)seeded.push(null);for(let i=0;i<size/2;i++)this.matches.push({id:i,round:1,a:seeded[i],b:seeded[size-1-i],sa:null,sb:null});this.buildNextRounds();}
 ...
 get rounds(){return Array.from(new Set(this.matches.map(m=>m.round)));}
