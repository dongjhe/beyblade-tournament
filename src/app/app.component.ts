...
export class AppComponent {
 ...
 addBattleScore(side:'a'|'b',type:FinishType){if(!this.battleMatch||this.battleFinished)return;const add=this.finishPoints(type);if(side==='a')this.battleMatch.sa=(this.battleMatch.sa??0)+add;else this.battleMatch.sb=(this.battleMatch.sb??0)+add;this.battleRound++;this.battleMatch.battleRound=this.battleRound;this.resetLaunchFails();this.updateScore(this.battleMatch);}
 launchFail(side:'a'|'b'){if(!this.battleMatch||this.battleFinished)return;this.launchFails[side]++;if(this.launchFails[side]<2)return;const opponent=side==='a'?'b':'a';if(opponent==='a')this.battleMatch.sa=(this.battleMatch.sa??0)+1;else this.battleMatch.sb=(this.battleMatch.sb??0)+1;this.battleRound++;this.battleMatch.battleRound=this.battleRound;this.resetLaunchFails();this.updateScore(this.battleMatch);}
