...
export class AppComponent {
 ...
 openBattle(m:Match){if(!m.a||!m.b)return;this.battleMatch=m;this.battleRound=m.battleRound??1;this.resetLaunchFails();}
 closeBattle(){this.countdownAudio?.pause();this.countdownPlaying=false;if(this.battleMatch){this.battleMatch.battleRound=this.battleRound;this.save();}this.battleMatch=null;this.resetLaunchFails();}
 ...
 drawBattle(){if(this.battleFinished)return;/* 平手／重賽：同一局重新比賽，局數與發射失敗次數都保留 */}
 private resetLaunchFails(){this.launchFails={a:0,b:0};}
 private finishPoints(type:FinishType){return type==='spin'?1:type==='xtreme'?3:2;}
 resetBattle(){if(!this.battleMatch)return;this.battleMatch.sa=null;this.battleMatch.sb=null;this.battleRound=1;this.battleMatch.battleRound=1;this.resetLaunchFails();this.updateScore(this.battleMatch);}
