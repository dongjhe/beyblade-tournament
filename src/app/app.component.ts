...
interface Match { id:number; round:number; a:string|null; b:string|null; sa:number|null; sb:number|null; battleRound?:number; }
...
export class AppComponent {
 ...
 private makeLeague(){let id=0;const pool:Match[]=[];for(let i=0;i<this.players.length;i++)for(let j=i+1;j<this.players.length;j++)pool.push({id:id++,round:1,a:this.players[i],b:this.players[j],sa:null,sb:null,battleRound:1});this.matches=this.buildRestFriendlySchedule(pool).map((m,i)=>({...m,id:i}));}
 ...
 private makeKnockout(){const size=this.nextPow2(this.players.length);const seeded:(string|null)[]=[...this.players];while(seeded.length<size)seeded.push(null);const draw=this.shuffle(seeded);for(let i=0;i<size;i+=2)this.matches.push({id:this.matches.length,round:1,a:draw[i],b:draw[i+1],sa:null,sb:null,battleRound:1});this.buildNextRounds();}
 private nextPow2(n:number){let x=1;while(x<n)x*=2;return x;}
 openBattle(m:Match){if(!m.a||!m.b)return;this.battleMatch=m;this.battleRound=m.battleRound??1;this.resetLaunchFails();this.cameraError='';}
 closeBattle(){this.countdownAudio?.pause();this.countdownPlaying=false;this.stopRecording();this.stopCamera();if(this.battleMatch){this.battleMatch.battleRound=this.battleRound;this.save();}this.battleMatch=null;this.resetLaunchFails();}
 battlePoints(side:'a'|'b'){if(!this.battleMatch)return 0;return side==='a'?(this.battleMatch.sa??0):(this.battleMatch.sb??0);}
 get battleFinished(){return !!this.battleMatch&&((this.battleMatch.sa??0)>=4||(this.battleMatch.sb??0)>=4);}
 ...
 private async startRecording(){if(this.recording)return true;if(!(await this.ensureCamera())||!this.cameraStream)return false;if(typeof MediaRecorder==='undefined'){this.cameraError='此瀏覽器不支援網頁錄影';return false;}try{if(this.recordingUrl){URL.revokeObjectURL(this.recordingUrl);this.recordingUrl='';}this.recordedChunks=[];const types=['video/mp4;codecs=h264,aac','video/mp4','video/webm;codecs=vp9,opus','video/webm'];const mimeType=types.find(t=>MediaRecorder.isTypeSupported(t));this.mediaRecorder=mimeType?new MediaRecorder(this.cameraStream,{mimeType}):new MediaRecorder(this.cameraStream);this.mediaRecorder.ondataavailable=e=>{if(e.data.size)this.recordedChunks.push(e.data);};this.mediaRecorder.onstop=()=>{if(!this.recordedChunks.length)return;const type=this.mediaRecorder?.mimeType||this.recordedChunks[0].type||'video/webm';this.recordingUrl=URL.createObjectURL(new Blob(this.recordedChunks,{type}));};this.mediaRecorder.start(250);this.recording=true;return true;}catch(e){console.error(e);this.cameraError='錄影啟動失敗';return false;}}
 stopRecording(){if(this.mediaRecorder&&this.mediaRecorder.state!=='inactive')this.mediaRecorder.stop();this.recording=false;}
 private stopCamera(){this.cameraStream?.getTracks().forEach(t=>t.stop());this.cameraStream=undefined;this.cameraReady=false;}
 downloadRecording(){if(!this.recordingUrl)return;const a=document.createElement('a');a.href=this.recordingUrl;a.download=`beyblade-${Date.now()}.${this.mediaRecorder?.mimeType.includes('mp4')?'mp4':'webm'}`;a.click();}
 async playCountdown(){if(this.countdownPlaying||this.battleFinished)return;await this.startRecording();try{this.countdownPlaying=true;if(!this.countdownAudio){const response=await fetch('assets/audio/321-go-shoot.mp3.b64');if(!response.ok)throw new Error('countdown audio not found');const base64=(await response.text()).trim();const binary=atob(base64);const bytes=new Uint8Array(binary.length);for(let i=0;i<binary.length;i++)bytes[i]=binary.charCodeAt(i);const url=URL.createObjectURL(new Blob([bytes],{type:'audio/mpeg'}));this.countdownAudio=new Audio(url);this.countdownAudio.preload='auto';this.countdownAudio.onended=()=>this.countdownPlaying=false;this.countdownAudio.onerror=()=>this.countdownPlaying=false;}this.countdownAudio.currentTime=0;await this.countdownAudio.play();}catch(e){console.error(e);this.countdownPlaying=false;}}
 addBattleScore(side:'a'|'b',type:FinishType){if(!this.battleMatch||this.battleFinished)return;const add=this.finishPoints(type);if(side==='a')this.battleMatch.sa=(this.battleMatch.sa??0)+add;else this.battleMatch.sb=(this.battleMatch.sb??0)+add;const finished=this.battleFinished;if(!finished)this.battleRound++;else this.stopRecording();this.battleMatch.battleRound=this.battleRound;this.resetLaunchFails();this.updateScore(this.battleMatch);}
 launchFail(side:'a'|'b'){if(!this.battleMatch||this.battleFinished)return;this.launchFails[side]++;if(this.launchFails[side]<2)return;const opponent=side==='a'?'b':'a';if(opponent==='a')this.battleMatch.sa=(this.battleMatch.sa??0)+1;else this.battleMatch.sb=(this.battleMatch.sb??0)+1;const finished=this.battleFinished;if(!finished)this.battleRound++;else this.stopRecording();this.battleMatch.battleRound=this.battleRound;this.resetLaunchFails();this.updateScore(this.battleMatch);}
 drawBattle(){if(this.battleFinished)return;}
 private resetLaunchFails(){this.launchFails={a:0,b:0};}
 private finishPoints(type:FinishType){return type==='spin'?1:type==='xtreme'?3:2;}
 resetBattle(){if(!this.battleMatch)return;this.stopRecording();this.battleMatch.sa=null;this.battleMatch.sb=null;this.battleRound=1;this.battleMatch.battleRound=1;this.resetLaunchFails();this.updateScore(this.battleMatch);}
 ...
 updateScore(m:Match){m.sa=m.sa===null?null:Math.max(0,Number(m.sa));m.sb=m.sb===null?null:Math.max(0,Number(m.sb));m.battleRound=this.battleRound;if(this.mode==='knockout'){this.matches=this.matches.filter(x=>x.round<=m.round);this.buildNextRounds();}this.save();}
 ...
 bracketMatches(r:number){const existing=this.roundMatches(r);return Array.from({length:this.bracketSlotCount(r)},(_,i)=>existing[i]??({id:-(r*100+i),round:r,a:null,b:null,sa:null,sb:null,battleRound:1} as Match));}
