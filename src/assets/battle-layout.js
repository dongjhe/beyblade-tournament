(() => {
  ...
  if (NativeMediaRecorder && !window.__beybladeRecorderPatched) {
    ...
    NativeMediaRecorder.prototype.start = function (...args) { activeRecorder = this; return nativeStart.apply(this, args); };
  ...
  style.textContent = `
    ...
    .battle-toolbar{width:100%;display:grid;grid-template-columns:minmax(0,1.35fr) minmax(0,1fr) minmax(0,.8fr);gap:8px;align-items:stretch;margin:0 0 4px}
    .battle-toolbar .countdown-btn,.battle-toolbar button{width:100%!important;max-width:none!important;min-width:0!important;min-height:48px;padding:8px 6px;border-radius:12px;font-size:12px;font-weight:900;white-space:nowrap}
    ...
    @media(max-width:760px) and (orientation:portrait){
      ...
      .battle-center{grid-area:center!important;width:100%!important;min-width:0!important}
      .battle-side{min-width:0!important}
      .battle-side.visual-left{grid-area:left!important}.battle-side.visual-right{grid-area:right!important}
      .battle-toolbar{grid-template-columns:minmax(0,1.45fr) minmax(0,.85fr) minmax(0,.7fr)!important;gap:6px!important;padding-right:46px;box-sizing:border-box}
      .battle-toolbar .countdown-btn,.battle-toolbar button{min-height:44px!important;padding:7px 4px!important;font-size:10px!important;line-height:1.15!important}
      .swap-side-btn{font-size:10px!important}.video-pause-btn{font-size:10px!important}
      .battle-round{margin-top:4px!important}
      .camera-preview,.replay-panel{width:min(340px,92vw)!important}
    }

    @media (orientation:landscape) and (max-height:600px){
      .battle-overlay{padding:0!important;align-items:flex-start!important}.battle-modal{width:100vw!important;max-width:none!important;min-height:100dvh!important;max-height:none!important;border-radius:0!important}
      .battle-arena{grid-template-columns:minmax(190px,.9fr) minmax(330px,1.35fr) minmax(190px,.9fr)!important;grid-template-areas:none!important;gap:12px!important;padding:12px 18px 14px!important}
      .battle-side.visual-left{grid-column:1!important;grid-row:1!important}.battle-center{grid-column:2!important;grid-row:1!important}.battle-side.visual-right{grid-column:3!important;grid-row:1!important}
      .battle-side{gap:5px!important}.battle-name{font-size:18px!important;padding:8px 5px!important;border-bottom-width:4px!important}.finish-btn{min-height:48px!important;padding:7px 10px!important}.finish-btn b,.finish-btn.xtreme b{font-size:16px!important}.finish-btn strong{font-size:19px!important}
      .battle-round{font-size:28px!important}.battle-sub{display:none!important}.battle-toolbar{margin-top:0!important;padding-right:42px!important;box-sizing:border-box}.battle-toolbar .countdown-btn,.battle-toolbar button{min-height:42px!important;padding:6px 4px!important;font-size:10px!important}
      .camera-preview,.replay-panel{position:static!important;width:100%!important;max-width:none!important;top:auto!important}.camera-preview{max-height:180px!important}.replay-video{max-height:175px!important}
      .score-versus{margin:2px 0!important}.big-score{height:62px!important;font-size:38px!important;border-width:3px!important;border-radius:14px!important}.score-versus small{margin-top:2px!important;font-size:7px!important}.draw-btn{padding:7px!important;font-size:12px!important}
      .battle-side .launch-fail-btn{min-height:45px!important;padding:6px!important;font-size:11px!important}.fail-marks i{width:16px!important;height:6px!important}.fail-note{display:none!important}.battle-footer{padding:6px 16px calc(6px + env(safe-area-inset-bottom))!important}
    }
  `;
  ...
  function pauseResumeRecording(button){
    ...
    if(recorder.state==='recording'){recorder.pause();button.classList.add('paused');button.textContent='▶ 繼續'}
    else if(recorder.state==='paused'){recorder.resume();button.classList.remove('paused');button.textContent='⏸ 暫停'}
  }
  ...
  function setupBattle(modal){
    ...
    pause.addEventListener('click',e=>{e.stopPropagation();pauseResumeRecording(pause)});
    swap.addEventListener('click',e=>{
      e.stopPropagation();swapped=!swapped;scoreVersus.classList.toggle('swapped',swapped);
      ...
      if(swapped){arena.insertBefore(playerB,center);arena.appendChild(playerA)}else{arena.insertBefore(playerA,center);arena.appendChild(playerB)}
    });
  }
